const { rankIndex } = require("./rankBonusConfig")
...
/** URL del pin configurado en admin para un rango (nombre motor o etiqueta). */
function resolveRankImageUrl(rankImagesDoc, rankName) {
  if (!rankImagesDoc || !rankName) return null
  const idx = rankIndex(rankName)
  const key = getHistoricalRankImageKey(idx)
  if (!key) return null
  const url = rankImagesDoc[key]
