...
/** Niveles de profundidad (desde el usuario) con al menos un socio con actividad. */
function countActiveDepthLevels(userId, usersById, treeById) {
  const levelsWithActivity = new Set()
  ...
  function walk(id, depth) {
    ...
    for (const childId of node.childs || []) {
      ...
      if (!child || isEliminated(child)) continue
      const hasActivity =
        Number(child.points || 0) > 0 ||
        Number(child.affiliation_points || 0) > 0 ||
        Number(child.total_points || 0) > 0
      if (hasActivity) levelsWithActivity.add(depth)
      walk(childId, depth + 1)
    }
  }

  walk(userId, 1)
  return levelsWithActivity.size
}

/** Niveles de profundidad máximos (desde el usuario) donde la organización tiene personas registradas. */
function countTotalDepthLevels(userId, usersById, treeById) {
  let maxDepth = 0
...
function calculateRankForUser(user, usersById, treeById, memo) {
  ...
  for (const r of RANK_CONFIG) {
    const isActivoRank = r.rank === "ACTIVO"
    const passesReconsumo = (skipReconsumo && isActivoRank) || reconsumo >= r.reconsumoRequired

    if (!passesReconsumo) continue
...
function computeRankProgress(user, usersList, treeList) {
  ...
  const bronceIdx = rankIndex("BRONCE")

  let targetRank = null
  let targetConfig = null
  // Activo no es objetivo de progreso: el primer rango a alcanzar es siempre Bronce
  if (currentIdx < bronceIdx) {
    targetRank = "BRONCE"
    targetConfig = RANK_CONFIG_BY_NAME.BRONCE
  } else if (currentIdx < RANK_ORDER.length - 1) {
  ...
  const reconsumo = reconsumoPoints(user)
  const activeLines = legs.length
  const activeLevels = countActiveDepthLevels(user.id, usersById, treeById)
  const totalDepthLevels = countTotalDepthLevels(user.id, usersById, treeById)
  const skipReconsumo = !!(user._activated || user.activated)
  ...
  const requirements = [
    {
      ...
      met: (skipReconsumo && targetConfig.rank === "ACTIVO") || reconsumo >= targetConfig.reconsumoRequired,
  ...
  return {
    historicRank: user.rank || "none",
    historicRankLabel: displayLabel(user.rank || "none"),
    currentRank,
    ...
    reconsumoMet: (skipReconsumo && targetConfig.rank === "ACTIVO") || reconsumo >= targetConfig.reconsumoRequired,
